...
public class MultiStateButtonField extends Field {
	...
	protected boolean navigationUnclick(int status,
            int time) {
        Utilities.log("XXXX " + Thread.currentThread().getName() + " : " +"navigationUnclick");
	...
	protected boolean touchEvent(TouchEvent message) {
        Utilities.log("XXXX " + Thread.currentThread().getName() + " : " +"touchEvent");
        if (message.getY(1) < 0) {
            return false;
        }
		...
		if (message.getEvent() == TouchEvent.UP && config.getState(current_state).getbmp_focused() != null) {
			...
			invalidate();			
//	        Utilities.log("XXXX " + Thread.currentThread().getName() + " : " +"touchEvent executing command");
//			command.execute(null, this);
			return true;
		}
		return false;
